...
const QUICK_ACTIONS = [
  { icon: Route, label: "Best Route Now" },
  { icon: ClipboardList, label: "Trip Protocol Reminder" },
...
export default function DriverAI() {
  const [messages, setMessages] = useState([
    {
      from: "ai",
      text: "Hi, I'm your Nirvaan driver assistant. I can help with routes, protocols, or nearby hospitals.",
      time: "10:42 AM",
    },
  ]);
  ...
  function handleSend(e) {
    ...
    setMessages((prev) => [
      ...
      { from: "ai", text: "Placeholder response — real assistant logic connects here in Phase 2+.", time: currentTime() },
    ]);
    setInput("");
  }
  ...
  function handleQuickAction(label) {
    setMessages((prev) => [
      ...
      { from: "ai", text: `Placeholder response for "${label}".`, time: currentTime() },
    ]);
  }
  ...
  return (
    <div className="min-h-screen bg-nirvaan-bg pb-40 flex flex-col max-w-md mx-auto md:max-w-lg">
      ...
      <div className="flex flex-col items-center mt-4 px-6 mb-4">
        ...
        <h2 className="text-xl font-extrabold text-nirvaan-dark">Driver Assistant</h2>
      </div>
      ...
      <div className="px-4 mt-5 space-y-2">
        {QUICK_ACTIONS.map((qa) => (
          <button
            ...
            className="w-full text-left px-4 py-3 rounded-full border-2 border-nirvaan-secondary text-nirvaan-secondary bg-white font-semibold text-sm flex items-center gap-2"
          >
      ...
      <form onSubmit={handleSend} className="fixed bottom-16 left-0 right-0 max-w-md mx-auto md:max-w-lg bg-nirvaan-bg px-4 py-3 border-t border-nirvaan-surface-high">
        <div className="flex items-center gap-2 bg-white border border-nirvaan-outline-variant rounded-full px-3 py-2">
          ...
          />
          <button type="submit" className="w-9 h-9 rounded-full bg-nirvaan-secondary text-white flex items-center justify-center flex-shrink-0">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-nirvaan-surface-high flex justify-around py-2.5 max-w-md mx-auto md:max-w-lg">
        ...
        <Link to="/driver/ai" className="flex flex-col items-center gap-0.5 text-nirvaan-secondary text-xs font-semibold"><span className="w-9 h-9 rounded-full bg-nirvaan-secondary text-white flex items-center justify-center"><Bot className="w-4 h-4" /></span>Assistant</Link>
