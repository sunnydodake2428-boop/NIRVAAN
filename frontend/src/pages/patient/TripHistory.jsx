...
import { Ambulance, PhoneCall, Home, Bot, History as HistoryIcon, User } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-nirvaan-bg pb-24 max-w-md mx-auto md:max-w-lg">
      ...
      <div className="px-4 pt-4">
        <h2 className="text-2xl font-extrabold text-nirvaan-dark mb-4">Trip History</h2>

        {loading && <p className="text-sm text-nirvaan-outline font-medium">Loading...</p>}
        {!loading && trips.length === 0 && (
          <p className="text-sm text-nirvaan-outline">No trips yet. Your requests will show up here.</p>
        )}
          ...
          {trips.map((t) => (
            <li key={t.id} className="bg-white rounded-xl p-4 shadow-sm border border-nirvaan-surface-high">
              <div className="flex items-center justify-between mb-1 gap-3">
                <p className="font-bold text-nirvaan-dark">
                  {t.pickup_address || "Pickup location"}
                </p>
                <span
                  className={`text-xs font-bold px-2.5 py-1 rounded-full capitalize whitespace-nowrap ${
                    STATUS_STYLES[t.status] || "bg-nirvaan-surface text-nirvaan-outline"
                  }`}
                >
                  {t.status}
                </span>
              </div>
              <p className="text-xs text-nirvaan-outline">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-nirvaan-surface-high flex justify-around py-2.5 max-w-md mx-auto md:max-w-lg">
        ...
        <Link to="/patient/history" className="flex flex-col items-center gap-0.5 text-nirvaan-secondary text-xs font-semibold">
          <span className="w-9 h-9 rounded-full bg-nirvaan-secondary text-white flex items-center justify-center"><HistoryIcon className="w-4 h-4" /></span>History
        </Link>
